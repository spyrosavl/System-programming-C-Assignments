...
List<unsigned long*>::~List() {
    for (int i = 0; i < len; ++i) {
        if(Array[i]!=NULL){
            if(!this->noDeleteNodes)free(Array[i]); //free tin domi pou evala mesa
            Array[i] = NULL;
        }
    }
    free(Array);
...
template <>
bool List<char *>::exists(char * query){
    for (int i = 0; i < this->getLen(); ++i) {
        if(strcmp(this->value(i),query)==0)return true;
    }
    return false;
}
...
template <class Items>
Items List<Items>::remove(){
    assert(len>0);
    Items data = this->value(len-1);
    len-=1;
    return data;
}
