...
#ifndef HW2_EXECUTOR_FIFOS_H
#define HW2_EXECUTOR_FIFOS_H

#define MAXBUFF 4000
#define FIFO "/tmp/syspro.hw2.fifo"
#define PERMS   0666

bool create_fifos(int pid){
    char fifo_name[80];
    int writefd;
...
bool unlink_fifos(int pid){
    ...
    if ( unlink(fifo_name) < 0) {
        perror("jobExecutor: can't unlink \n");
    }
    sprintf(fifo_name, "%s.%d_slave",FIFO, pid);
    if ( unlink(fifo_name) < 0) {
        perror("jobExecutor: can't unlink \n");
...
bool write_to_pipe(int writefd, char * message){
    if (write(writefd, message, strlen(message)) == -1)
        perror("jobExecutor: filename write error");
}
